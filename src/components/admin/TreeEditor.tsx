...
import type { TreeNode, NodeType, ExperienceLevel } from '../../types/cms';
import { 
  Folder, 
  ChevronRight, 
  ChevronDown, 
...
export const TreeEditor: React.FC<TreeEditorProps> = ({
  ...
  const updateNodeRecursive = (node: TreeNode, targetId: string, mutator: (n: TreeNode) => TreeNode): TreeNode => {
    ...
    if (node.children) {
      return {
        ...node,
        children: node.children.map(child => updateNodeRecursive(child, targetId, mutator))
      };
    }
    return node;
  };
  ...
  const handleDuplicateNode = (nodeId: string) => {
    ...
    const addSiblingRecursive = (parent: TreeNode): TreeNode => {
      ...
      if (parent.children) {
        return { ...parent, children: parent.children.map(addSiblingRecursive) };
      }
      return parent;
    };

    const updatedTree = addSiblingRecursive(treeData);
    pushHistory(updatedTree);
  };

  // Move node up/down in sibling array
  const handleMoveSibling = (nodeId: string, direction: 'up' | 'down') => {
    const moveRecursive = (parent: TreeNode): TreeNode => {
      ...
      if (parent.children) {
        return { ...parent, children: parent.children.map(moveRecursive) };
      }
      return parent;
    };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      {/* RIGHT COLUMN: NODE PROPERTIES PANEL (5 Cols) */}
          ...
          <div className="space-y-4 text-xs font-mono">
            <div>
              ...
              <input
                ...
                onChange={e => {
                  const updated = updateNodeRecursive(treeData, selectedNode.id, n => ({ ...n, name: e.target.value }));
                  pushHistory(updated);
                }}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  onChange={e => {
                    const updated = updateNodeRecursive(treeData, selectedNode.id, n => ({ ...n, type: e.target.value as NodeType }));
                    pushHistory(updated);
                  }}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => {
                    const updated = updateNodeRecursive(treeData, selectedNode.id, n => ({ ...n, experienceLevel: e.target.value as ExperienceLevel }));
                    pushHistory(updated);
                  }}
                  className="w-full px-2.5 py-1.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white"
                >
            ...
            <div>
              ...
              <input
                ...
                onChange={e => {
                  const updated = updateNodeRecursive(treeData, selectedNode.id, n => ({ ...n, color: e.target.value }));
                  pushHistory(updated);
                }}
            ...
            <div>
              ...
              <textarea
                ...
                onChange={e => {
                  const updated = updateNodeRecursive(treeData, selectedNode.id, n => ({ ...n, description: e.target.value }));
                  pushHistory(updated);
                }}
